# Design contract — C-PLANS-LIFECYCLE-DESKTOP-252-V7

## Product boundary

This change improves only the desktop presentation of the existing plan lifecycle register. It creates no lifecycle value, client-side list, archive action, stored preference or mobile navigation flow.
...
The existing `#plans-lifecycle-status` select remains the single state and compatibility authority. The desktop segment changes that select and emits its ordinary `input` and `change` events. API requests, selection reconciliation and rendering continue through the established lifecycle controller.

## Narrow and mobile fallback

Below `721 px` the desktop segment is removed. The source select and its owner are restored with their exact pre-enhancement `hidden`, `aria-hidden` and `tabindex` state. No mobile list/detail mode, bottom sheet, gesture, mobile-only navigation or mobile-only animation is introduced.
...
- One click, Enter or Space changes the mode through native button behavior.
- Left/Right arrows, Home and End move focus and activate the corresponding tab.
