...
5. перезагрузить страницу и убедиться, что сводка восстановилась из устойчивых данных;
...
Release workflow дополнительно воспроизводит forced rollback на том же собранном artifact; отдельная пересборка для rollback evidence запрещена.
