...
запускается после успешного `Release gate` для `main`. Он ждёт остальные
обязательные post-merge workflows для того же SHA, повторно проверяет исходный
код и recovery path, собирает/проверяет full bundle и Project Control package,
а затем создаёт tag и GitHub Release с контрольными суммами. Stable promotion
по-прежнему требует реальной Astra/Debian приёмки из
[TARGET_ACCEPTANCE.md](TARGET_ACCEPTANCE.md) и issue #27.
