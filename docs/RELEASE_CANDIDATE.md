...
`0.1.0-rc.8` — текущий эксплуатационный release candidate, схема SQLite **19**. Основные функциональные контуры работают автономно и проходят unit/integration/Chromium/full-offline gates. Stable не объявляется до фактической приёмки на целевых Astra Linux/Debian по [`TARGET_ACCEPTANCE.md`](TARGET_ACCEPTANCE.md) и issue #27.

`rc.8` добавляет гарантированный запуск с ранней проверкой Node.js runtime до любых изменений файлов, проверку совместимости серий Astra Linux (1.7.x / 1.8.x), автоматическую самодиагностику и восстановление пакетов (`doctor.sh --repair`, автоматическое безопасное завершение конфигурации `dpkg --configure -a`).

## Что входит в rc.8

- гарантированный старт и прямая проверка выполнения Node.js runtime на ранней стадии;
- поддержка совместимости всех версий обновлений Astra Linux 1.7.x и 1.8.x;
