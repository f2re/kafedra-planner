...
- **Учебный процесс** — ведомости групп, учебные годы, семестры, оценки и сводная успеваемость;
...
В разделе **Учебный процесс → Успеваемость** оператор загружает XLSX, ODS или CSV-ведомость, где строки соответствуют студентам, а столбцы — дисциплинам. Исходный файл и его версия остаются неизменяемыми.

Система автоматически предлагает лист, строку заголовков, колонку ФИО, дисциплины, учебную группу, учебный год и семестр. Для каждого метаполя оператор видит и выбирает один из двух доказуемых вариантов:

- **Из ячейки** — конкретный лист и адрес, например `Таблица!B3`; сервер повторно читает значение из сохранённой версии документа;
- **Ввести вручную** — явное значение без фиктивной ссылки на ячейку.

Рабочий список сгруппирован как `учебный год → семестр → группа`. Для каждой группы в сводку входит только последняя успешно обработанная ведомость; предыдущие и ошибочные версии остаются в истории и не удваивают показатели.

По каждой дисциплине рассчитываются количество оценок `отлично`, `хорошо`, `удовлетворительно`, `неудовлетворительно`, `не аттестован`, число неоднозначных значений и средний балл только по оценкам `2–5`. Доступны CSV, JSON и `sources.json`; строка раскрывается до студента, версии документа, листа и исходной ячейки.

Подробно: [`docs/ACADEMIC_PERFORMANCE.md`](docs/ACADEMIC_PERFORMANCE.md).
...
CI проверяет миграции до schema 031, backup/restore, Node 24.15 и host Node 25, desktop/mobile Chromium, интеграцию Оформлятора, full-offline Debian 12, additive APT policy и systemd-сценарии с LLM и без него.
...
- [`docs/ACADEMIC_PERFORMANCE.md`](docs/ACADEMIC_PERFORMANCE.md) — ведомости и сводка успеваемости;
