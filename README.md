...
> Текущий рубеж: **`0.4.4`**, схема SQLite **31**. Основные пользовательские сценарии работают без обязательного Интернета, Docker, LLM, Оформлятора и облачных сервисов. До stable остаётся фактическая эксплуатационная приёмка Astra Linux/Debian по [`docs/TARGET_ACCEPTANCE.md`](docs/TARGET_ACCEPTANCE.md) и issue #27.

Patch release `0.4.4` завершает цикл упрощения R1–R9 после `0.4.3`: OCR сохраняет абсолютные страницы и допускает повторную обработку того же неизменяемого источника, загрузка честно показывает частичный успех, заседания и путь `календарь → план` требуют меньше предварительных действий, периодические задачи выполняются напрямую, поиск открывает точный рабочий объект, а персональные подстановки остаются под контролем пользователя. Разработка и выпуск также упрощены без ослабления offline install/update/rollback.
...
GitHub Release и его assets неизменяемы. `v0.4.1`–`v0.4.3` остаются историческими выпусками; изменения R1–R9 относятся к `v0.4.4`.
