...
> Текущий рубеж: **`0.4.3`**, схема SQLite **31**. Основные пользовательские сценарии работают без обязательного Интернета, Docker, LLM, Оформлятора и облачных сервисов. До stable остаётся фактическая эксплуатационная приёмка Astra Linux/Debian по [`docs/TARGET_ACCEPTANCE.md`](docs/TARGET_ACCEPTANCE.md) и issue #27.

Patch release `0.4.3` добавляет пакетную загрузку протоколов за выбранный год, независимую обработку каждого файла, устойчивую сводку `готово / проверить / ошибка / обрабатывается` и исправление только сомнительных полей без уничтожения исходного extraction/evidence. Release pipeline теперь использует один универсальный gate и публикует тот же offline artifact, который прошёл install/update/rollback verification.
...
GitHub Release и его assets неизменяемы. Старые `v0.4.1` и `v0.4.2` остаются историческими выпусками; новые изменения не подмешиваются в прежние bundle. Годовой импорт протоколов и упрощённый универсальный release pipeline входят в `v0.4.3`.
...
Обычный PR проходит проектный CI по риску изменения. Тяжёлый **Release gate** запускается только на `main` перед публикацией: он выполняет полный unit/integration regression, критический desktop/mobile browser и recovery checks. Publisher не повторяет эти suites: он один раз собирает offline bundle, проверяет install/update/forced rollback именно этого artifact и публикует его без пересборки.
