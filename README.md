...
Patch release `0.4.3` добавляет пакетную загрузку протоколов за выбранный год, независимую обработку каждого файла, устойчивую сводку `готово / проверить / ошибка / обрабатывается` и исправление только сомнительных полей без уничтожения исходного extraction/evidence. Выпуск выполняется одним ручным workflow `Release`, который проверяет и публикует тот же offline artifact, прошедший install/update/rollback.
...
GitHub Release и его assets неизменяемы. Старые `v0.4.1` и `v0.4.2` остаются историческими выпусками; новые изменения не подмешиваются в прежние bundle. Годовой импорт протоколов относится к `v0.4.3`.
