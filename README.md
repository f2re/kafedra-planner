...
> Текущий рубеж: **`0.4.7`**, схема SQLite **31**. Основные пользовательские сценарии работают без обязательного Интернета, Docker, LLM, Оформлятора и облачных сервисов. До stable остаётся фактическая эксплуатационная приёмка Astra Linux/Debian по [`docs/TARGET_ACCEPTANCE.md`](docs/TARGET_ACCEPTANCE.md) и issue #27.

Patch release `0.4.7` добавляет поиск по теме, ограниченную коррекцию написания, похожие запросы и необязательный фоновый подбор через локальную ЛЛМ. Модель может расширить поиск за пределы первоначальной выдачи; источники и выдержки проверяются, а обычный поиск не ждёт генерации. Автоматическая вставка в официальный отчёт не входит в этот рубеж. Отдельные автономные комплекты Debian 12, Astra Linux 1.7 и 1.8 с OCR/PDF/Office-компонентами своей ОС из `0.4.6` сохранены.
...
GitHub Release и его assets не перезаписываются. Изменения R1–R9 относятся к `v0.4.4`, развитие импорта протоколов — к `v0.4.5`, поставка отдельных Astra-комплектов — к `v0.4.6`, поиск по теме и фоновый подбор — к `v0.4.7`.
...
- **Заседания** — повестка, решения, пакетный импорт протоколов за год, выписки и версии DOCX-шаблонов.
...
Введите тему, например «отчёт где была статья про циклоны». Поиск использует текущий индекс документов и рабочих объектов; явные фильтры сохраняются. При включённой ЛЛМ дополнительный подбор предлагает похожие формулировки и реальные источники с проверяемыми выдержками. Выдача не переставляется; материал открывается в своей рабочей карточке, а **К поиску** возвращает исходный контекст.

Подсказки можно отключить в текущей вкладке. Недоступность модели не мешает основному поиску. Сходство темы не подтверждает выполнение или публикацию. Подробно: [`docs/SEMANTIC_SEARCH.md`](docs/SEMANTIC_SEARCH.md), [`docs/SEARCH_LLM_PROMPTS.md`](docs/SEARCH_LLM_PROMPTS.md), [`docs/BUREAUCRACY_ASSISTANT.md`](docs/BUREAUCRACY_ASSISTANT.md).
...
В **Заседаниях** можно выбрать календарный год и одним действием загрузить все протоколы DOCX/ODT/PDF/TXT. Каждый исходник сохраняется до интерпретации и обрабатывается независимо. Надёжно распознанные заседания доступны сразу; сомнительный номер, дата, вопрос, решение, ответственный или срок остаются в очереди проверки и исправляются в рабочей карточке. Ручная правка не стирает машинный результат, locator или SHA-256 исходника.
...
Подробно: [`docs/PLANS.md`](docs/PLANS.md), [`docs/SIMPLE_COMPLETION.md`](docs/SIMPLE_COMPLETION.md), [`docs/MEETINGS.md`](docs/MEETINGS.md), [`docs/PROTOCOL_IMPORT.md`](docs/PROTOCOL_IMPORT.md).

## Установка и обновление

На целевой машине не требуются `npm install` или `pip install`. Скачайте архив для своей ОС, его `.sha256`, `install-kafedra-planner.sh` и `README-INSTALL.txt` из одного GitHub Release в обычную читаемую папку пользователя:

```bash
sudo KAFEDRA_APT_MODE=bundle bash ./install-kafedra-planner.sh
```

Если рядом несколько архивов, установщик выбирает единственный совместимый по встроенному профилю ОС и архитектуре. Отсутствие или неоднозначность выбора останавливает установку до изменений системы. Debian-архив не заменяет Astra-архив.

Владельцем папки загрузки и исходного архива не должен быть `root`. Wrapper проверяет внешний SHA-256 и внутренний manifest, извлекает bundle в приватный root staging, ставит только отсутствующие document capabilities, проверяет Tesseract `rus+eng`, Poppler и LibreOffice, создаёт и проверяет резервную копию, атомарно переключает `/opt/kafedra-planner/current` и выполняет rollback при ошибке.
...
Обычный pull request выполняет полный locked install, check, docs, unit/integration и smoke один раз. После merge в `main` запускается только короткий post-merge smoke. Targeted Playwright запускается для затронутого UI-сценария. Полный browser, backup/restore и offline/systemd install-update-rollback относятся только к профильному риск-контуру или к явному workflow `Release`.
