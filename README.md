...
Эксплуатационная установка не требует `npm install` или `pip install` на целевой машине. Для Debian 12 `amd64` скачайте четыре файла из [GitHub Releases](https://github.com/f2re/kafedra-planner/releases), проверьте `.sha256` и запустите штатный wrapper по [короткой инструкции](docs/GITHUB_RELEASES.md).

Full bundle для Astra Linux, другой архитектуры или иной серии ОС собирается на совместимой reference-системе:
