...
> Текущий рубеж: **`0.4.6`**, схема SQLite **31**. Основные пользовательские сценарии работают без обязательного Интернета, Docker, LLM, Оформлятора и облачных сервисов. До stable остаётся фактическая эксплуатационная приёмка Astra Linux/Debian по [`docs/TARGET_ACCEPTANCE.md`](docs/TARGET_ACCEPTANCE.md) и issue #27.

Patch release `0.4.6` добавляет отдельные автономные комплекты для Debian 12, Astra Linux 1.7 и 1.8, автоматический выбор совместимого архива и проверку обновления с предыдущего опубликованного выпуска. Каждый комплект содержит OCR/PDF/Office-компоненты своей ОС. Распознавание протоколов, массовая правка реквизитов, версии форматов, безопасный повтор, перенос вопросов и обратимый архив из `0.4.5` сохранены.
...
GitHub Release и его assets не перезаписываются. Изменения R1–R9 относятся к `v0.4.4`, развитие импорта протоколов — к `v0.4.5`, поставка отдельных Astra-комплектов — к `v0.4.6`.
