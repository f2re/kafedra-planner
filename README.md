...
> Текущий рубеж: **`0.4.5`**, схема SQLite **31**. Основные пользовательские сценарии работают без обязательного Интернета, Docker, LLM, Оформлятора и облачных сервисов. До stable остаётся фактическая эксплуатационная приёмка Astra Linux/Debian по [`docs/TARGET_ACCEPTANCE.md`](docs/TARGET_ACCEPTANCE.md) и issue #27.

Patch release `0.4.5` завершает текущую доработку заседаний после `0.4.4`: нечёткое распознавание протоколов, массовую правку реквизитов, версии форматов и безопасный повтор. Дата определяется из каждого загруженного документа. Неудачные загрузки можно убрать из списка или в обратимый архив, сохранив исходники и историю. Перенос вопросов и формирование DOCX сохраняют все решения и связи с планами. Реальная пользовательская проверка документов проводится после установки выпуска.
...
GitHub Release и его assets неизменяемы. `v0.4.1`–`v0.4.4` остаются историческими выпусками; изменения R1–R9 относятся к `v0.4.4`, развитие импорта протоколов — к `v0.4.5`.
