...
export function recordUiPreferences(database, workspaceId, accountId, body, now = new Date().toISOString()) {
  if (!accountId) return { recorded: 0, preferences: listUiPreferences(database, workspaceId, null) };
  const interactionId = String(body?.interactionId || '').trim();
  if (!/^[A-Za-z0-9._:-]{8,160}$/u.test(interactionId)) {
    throw new AppError('ui_preference_interaction_invalid', 'Не удалось однозначно определить пользовательское действие.', 400);
  }
  if (!Array.isArray(body?.choices) || !body.choices.length || body.choices.length > 32) {
    throw new AppError('ui_preference_choices_invalid', 'Передан некорректный набор пользовательских выборов.', 400);
  }
