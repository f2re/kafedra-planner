...
function today(now = new Date().toISOString()) { return String(now).slice(0, 10); }
function previousDay(value) { const date = new Date(`${value}T00:00:00Z`); date.setUTCDate(date.getUTCDate() - 1); return date.toISOString().slice(0, 10); }
...
export function safelyClosePreviousPrimary(database, workspaceId, input = {}, now = new Date().toISOString()) {
  ...
  const current = database.get(`SELECT * FROM person_appointments
    ...
    ORDER BY valid_from DESC LIMIT 1`, workspaceId, input.personId, validFrom, validFrom);
...
function currentPrimary(database, workspaceId, personId, at) {
  return database.get(`SELECT a.*, COALESCE(pos.name, a.position_title_snapshot) AS effective_position
    FROM person_appointments a LEFT JOIN organization_positions pos ON pos.id = a.position_id
    WHERE a.workspace_id = ? AND a.person_id = ? AND a.appointment_kind = 'primary'
      AND a.status <> 'cancelled' AND a.valid_from <= ? AND (a.valid_to IS NULL OR a.valid_to >= ?)
    ORDER BY a.valid_from DESC LIMIT 1`, workspaceId, personId, at, at) || null;
}

function currentManager(database, workspaceId, unitId, at) {
  if (!unitId) return null;
  return database.get(`SELECT m.* FROM organization_unit_managers m
    ...
    ORDER BY m.valid_from DESC LIMIT 1`, workspaceId, unitId, at, at) || null;
}
...
export function syncUnitManagerCompatibility(database, workspaceId, unitId, previousManagerId = null, now = new Date().toISOString()) {
  ...
  const people = database.all(`SELECT DISTINCT person_id FROM person_appointments
    WHERE workspace_id = ? AND organization_unit_id = ? AND appointment_kind = 'primary'
      AND manager_person_id IS NULL AND status <> 'cancelled'
      AND valid_from <= ? AND (valid_to IS NULL OR valid_to >= ?)`, workspaceId, unitId, at, at);
...
function publicationDate(database, scientificItemId) {
  ...
  return item ? String(item.published_at || (item.publication_year ? `${item.publication_year}-12-31` : '') || item.created_at || today()).slice(0, 10) : null;
}

export function refreshDerivedScientificAffiliations(database, workspaceId, personId, now = new Date().toISOString()) {
  const authors = database.all(`SELECT sia.scientific_item_id, sia.author_raw
    FROM scientific_item_authors sia JOIN scientific_items si ON si.id = sia.scientific_item_id
    WHERE si.workspace_id = ? AND sia.person_id = ?`, workspaceId, personId);
  for (const author of authors) {
    const validOn = publicationDate(database, author.scientific_item_id) || today(now);
    const appointment = database.get(`SELECT a.*, u.name AS unit_name, COALESCE(pos.name, a.position_title_snapshot) AS position_name
      FROM person_appointments a JOIN organization_units u ON u.id = a.organization_unit_id
      ...
      ORDER BY a.appointment_kind = 'primary' DESC, a.valid_from DESC LIMIT 1`, workspaceId, personId, validOn, validOn) || null;
    database.run(`INSERT INTO scientific_author_affiliations(
      ...
      ON CONFLICT(scientific_item_id, author_raw) DO UPDATE SET
        ...
        unit_name_snapshot = excluded.unit_name_snapshot, position_name_snapshot = excluded.position_name_snapshot,
    ...
    newId('scienceaff'), workspaceId, author.scientific_item_id, author.author_raw, personId,
    appointment?.id || null, appointment?.organization_unit_id || null, appointment?.position_id || null,
    appointment?.unit_name || null, appointment?.position_name || null, validOn,
    JSON.stringify({ source: 'organization_history', appointmentId: appointment?.id || null }), now, now);
  }
}
