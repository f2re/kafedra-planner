...
export function parseTesseractTsv(tsv, { page = 1 } = {}) {
  ...
  for (const row of rows.slice(1)) {
    ...
    const key = [
      pageNumber,
      columns[index.block_num] || 0,
      columns[index.par_num] || 0,
      columns[index.line_num] || 0
    ].join(':');
    const word = {
      text,
  ...
  for (const language of languageCandidates(languages)) {
    try {
      const { stdout } = await execFileAsync(
        'tesseract',
        [path, 'stdout', '-l', language, '--dpi', String(dpi), 'tsv'],
        { encoding: 'utf8', timeout: 180_000, maxBuffer: 128 * 1024 * 1024 }
      );
      const parsed = parseTesseractTsv(stdout, { page });
      return {
        status: parsed.blocks.length ? 'used' : 'empty',
        engine: 'tesseract',
        languages: language,
        confidence: parsed.confidence,
        text: parsed.text,
        blocks: parsed.blocks,
        error: parsed.blocks.length ? null : 'ocr_text_empty'
      };
    } catch (error) {
      lastError = error;
...
export async function ocrImage(path, options = {}) {
  if (options.enabled === false) {
    return {
      status: 'disabled',
      engine: null,
      languages: String(options.languages || ''),
      confidence: null,
      text: '',
      blocks: [],
      error: null
  ...
  return runTesseract(path, {
    languages: options.languages || 'rus+eng',
    dpi: options.dpi || 250,
    page: options.page || 1
  });
}
  ...
  try {
    ...
    try {
      await execFileAsync(
        ...
        [
          '-png',
          '-r', String(options.dpi || 250),
          '-f', '1',
          '-l', String(options.maxPages || 50),
          path,
          prefix
        ],
      ...
      return {
        ...
        text: '',
        blocks: [],
        error: `pdftoppm:${commandError(error)}`
      };
    }
    ...
    for (let index = 0; index < pages.length; index += 1) {
      const result = await runTesseract(join(directory, pages[index]), {
        languages: options.languages || 'rus+eng',
        dpi: options.dpi || 250,
        page: index + 1
      });
    ...
    const failed = pageResults.find((item) => ['unavailable', 'failed'].includes(item.status));
    return {
      status: used ? 'used' : failed?.status || 'empty',
      engine: 'tesseract',
      ...
      text: blocks.map((block) => block.text).join('\n'),
      blocks,
      error: used ? null : failed?.error || 'ocr_text_empty'
    };
  } finally {
