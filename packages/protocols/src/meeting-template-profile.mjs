...
function numberAttr(xml, tag, name = 'val') {
...
function paragraphStyle(paragraphXml) {
  ...
  const numId = numberAttr(properties, 'numId');
  const level = numberAttr(properties, 'ilvl');
  return {
    alignment: ['left', 'center', 'right', 'both', 'justify'].includes(alignment) ? alignment : null,
    marginLeftPt: twipsToPt(Number(attr(indent, 'left'))),
    marginRightPt: twipsToPt(Number(attr(indent, 'right'))),
    firstLinePt: twipsToPt(Number(attr(indent, 'firstLine'))),
    hangingPt: twipsToPt(Number(attr(indent, 'hanging'))),
    spaceBeforePt: twipsToPt(Number(attr(spacing, 'before'))),
    spaceAfterPt: twipsToPt(Number(attr(spacing, 'after'))),
    linePt: twipsToPt(Number(attr(spacing, 'line'))),
...
function cellStyle(cellXml) {
  ...
  const width = /<w:tcW\b[^>]*w:w="(-?\d+)"/iu.exec(properties)?.[1];
  const span = numberAttr(properties, 'gridSpan');
  const merge = /<w:vMerge\b([^>]*)/iu.exec(properties);
  const mergeValue = merge ? attr(merge[0], 'val') : null;
  const fill = /<w:shd\b[^>]*w:fill="([^"]+)"/iu.exec(properties)?.[1] || null;
  return {
    widthPt: twipsToPt(width === undefined ? null : Number(width)),
...
function parseLocator(elementId) {
  const body = new RegExp(`^${DOCUMENT_XML.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')}#body/p:(\\d+)$`, 'u').exec(elementId);
  if (body) return { kind: 'body_paragraph', paragraphIndex: Number(body[1]) };
  const table = new RegExp(`^${DOCUMENT_XML.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')}#table:(\\d+)/row:(\\d+)/cell:(\\d+)/p:(\\d+)$`, 'u').exec(elementId);
  if (table) {
    return {
...
export async function saveMeetingTemplateProfile(database, config, workspaceId, templateVersionId, documentKind, input, actorPersonId = null) {
  ...
  const blob = await storeIncomingStream(Readable.from([Buffer.from(serialized, 'utf8')]), {
    ...
    maxBytes: Math.min(config.maxUploadBytes, 2 * 1024 * 1024),
