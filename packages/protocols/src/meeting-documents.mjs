...
function resolveProfile(database, workspaceId, meeting, kind, templateVersionId) {
...
export async function generateMeetingDocument(database, config, workspaceId, meetingId, input, actorPersonId = null) {
  ...
  const profile = resolveProfile(database, workspaceId, meeting, kind, templateVersionId);
  ...
  let rendered;
  try {
    rendered = await renderMeetingDocumentFile({
      templatePath: template.storage_path,
      outputPath,
      model,
      profile: mode === 'template' && !profile ? null : profile
    });
