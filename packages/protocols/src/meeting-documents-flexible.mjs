...
import { listMeetingTemplateCatalog } from './meeting-template-library.mjs';
...
import { generateMeetingDocument as generateConfiguredMeetingDocument } from './meeting-documents.mjs';
...
function pinnedTemplateVersion(meeting, kind) {
  return kind === 'protocol'
    ? meeting.protocol_template_version_id || null
    : meeting.extract_template_version_id || null;
}
...
function selectableCatalog(database, workspaceId, kind) {
  return listMeetingTemplateCatalog(database, workspaceId, { kind })
    .filter((entry) => entry.lifecycle_status === 'active' && SELECTABLE_READINESS.has(entry.readiness));
}

function resolveFallbackTemplate(database, workspaceId, kind, explicitTemplateVersionId = null) {
  const entries = selectableCatalog(database, workspaceId, kind);
  if (explicitTemplateVersionId) {
    const explicit = entries.find((entry) => entry.document_version_id === explicitTemplateVersionId);
    if (explicit) return explicit.document_version_id;
    const known = listMeetingTemplateCatalog(database, workspaceId, { kind, includeArchived: true })
      .find((entry) => entry.document_version_id === explicitTemplateVersionId);
    if (known) fail(known.lifecycle_status === 'active'
      ? 'meeting_template_profile_incomplete'
      : 'meeting_template_catalog_archived');
    fail('meeting_template_catalog_not_found');
  }
  const preferred = entries.find((entry) => Number(entry.is_default) === 1);
  if (preferred) return preferred.document_version_id;
  if (entries.length === 1) return entries[0].document_version_id;
...
async function generateWithTemplate(database, config, workspaceId, meeting, input, actorPersonId, templateVersionId) {
  ...
  `, meeting.id, kind, requestHash);
  if (existing) return { ...existing, duplicateRequest: true };

  await mkdir(config.tempDir, { recursive: true });
  const outputPath = join(config.tempDir, `${meeting.id}-${kind}-${requestHash.slice(0, 16)}-${process.pid}-${newId('tmp')}.docx`);
  try {
    ...
    database.transaction(() => {
      ...
      if (kind === 'protocol') {
        database.run(`
          ...
          WHERE id = ?
        `, result.versionId, now, meeting.id);
...
export async function generateMeetingDocument(database, config, workspaceId, meetingId, input, actorPersonId = null) {
  ...
  const kind = clean(input?.kind) || 'protocol';
  const pinned = pinnedTemplateVersion(meeting, kind);
  const explicitTemplateVersionId = clean(input?.templateVersionId) || null;

  if (pinned || !explicitTemplateVersionId) {
    try {
      return await generateConfiguredMeetingDocument(database, config, workspaceId, meetingId, input, actorPersonId);
    } catch (error) {
      if (pinned || errorCode(error) !== 'meeting_settings_incomplete') throw error;
    }
  }

  const templateVersionId = resolveFallbackTemplate(
    database,
    workspaceId,
    kind,
    explicitTemplateVersionId
  );
  return generateWithTemplate(database, config, workspaceId, meeting, input, actorPersonId, templateVersionId);
}
