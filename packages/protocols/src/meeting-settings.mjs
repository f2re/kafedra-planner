...
function templateProfile(database, workspaceId, template, kind, readyOnly = true) {
  return latestMeetingTemplateProfile(database, workspaceId, template.version_id, kind, readyOnly);
}

function templateStructure(database, versionId) {
  return database.get(`
    SELECT structure_status FROM document_versions WHERE id = ?
  `, versionId)?.structure_status || null;
}
...
function resolvedTemplate(database, workspaceId, versionId, kind) {
  ...
  const profile = templateProfile(database, workspaceId, template, kind, true);
...
export function meetingSettingsResources(database, workspaceId) {
  ...
  `, workspaceId).map((template) => {
    const protocolProfile = latestMeetingTemplateProfile(database, workspaceId, template.version_id, 'protocol', false);
    const extractProfile = latestMeetingTemplateProfile(database, workspaceId, template.version_id, 'extract', false);
    return {
      ...template,
