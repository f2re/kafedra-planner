...
import {
  claimOpenPlanItem, createManualPlan, createManualPlanItem, setPlanItemExecution
...
function addDocument(database, workspaceId, id, versionId, originalName, {
  ...
  `, id, workspaceId, originalName.replace(/\.[^.]+$/, ''), versionId, now, now);
  database.run(`
  ...
  `, versionId, id, blob, originalName, mediaType, detectedFormat, extractedText, now);
  return { id, versionId };
}
...
test('обновление с schema 18 применяет ручные планы из 019 и все последующие миграции без потери источника', async () => {
  ...
  try {
    ...
    addDocument(database, workspace.id, 'doc_old_plan', 'ver_old_plan', 'old-plan.txt', {
      mediaType: 'text/plain', detectedFormat: 'text'
    });
    database.run(`
      INSERT INTO plans(
      ...
      ) VALUES ('plan_old',?,'ver_old_plan','department','calendar','2026',2026,2026,
        'Существующий план','active',1,'{}','2026-08-01T00:00:00Z','2026-08-01T00:00:00Z')
    `, workspace.id);
    database.run(`
      INSERT INTO plan_items(
        id,plan_id,source_item_key,item_no,title,direction,status,confidence,evidence_json,created_at,updated_at
      ) VALUES ('item_old','plan_old','row:1','1','Существующий пункт','organizational','planned',1,'{}',
...
test('ручной план замыкает календарь, поручение, self-claim, отчёт и сопроводительные документы', async () => {
  ...
  try {
    ...
    const plan = createManualPlan(database, workspace.id, {
      title: 'План кафедры 2026', planKind: 'department', periodKind: 'calendar', yearStart: 2026,
    ...
    const personal = createManualPlan(database, workspace.id, {
      title: 'Личный план', planKind: 'personal', periodKind: 'calendar', yearStart: 2026,
      ownerPersonId: manager.id
    }, manager.id, now);
    ...
    const tracked = createManualPlanItem(database, workspace.id, plan.id, {
      title: 'Заседание кафедры', startsAt: '2026-09-15', direction: 'organizational',
      executionMode: 'track', expectedResult: 'Протокол'
    }, manager.id, now);
    ...
    const assigned = database.transaction(() => createManualPlanItem(database, workspace.id, plan.id, {
      title: 'Подготовить отчёт', dueDate: '2026-10-20', direction: 'science',
      executionMode: 'assigned', executorPersonIds: [executor.id], responsiblePersonId: executor.id,
      controllerPersonId: manager.id, expectedResult: 'Отчёт 100%'
    }, manager.id, now));
    ...
    setPlanItemExecution(database, workspace.id, assigned.id, {
      executionMode: 'assigned', executorPersonIds: [executor.id], controllerPersonId: manager.id
    }, manager.id, '2026-08-17T09:01:00.000Z');
    assert.equal(database.get('SELECT COUNT(*) AS n FROM plan_item_assignments WHERE plan_item_id=?', assigned.id).n, 1);
    assert.equal(database.get('SELECT COUNT(*) AS n FROM assignments WHERE id=?', assignmentId).n, 1);

    const open = database.transaction(() => createManualPlanItem(database, workspace.id, plan.id, {
      title: 'Подготовить выставку', dueDate: '2026-11-10', direction: 'organizational',
      executionMode: 'open', controllerPersonId: manager.id
    }, manager.id, now));
    ...
    assert.equal(database.get(`
      SELECT COUNT(*) AS n FROM assignment_executors
      WHERE assignment_id=? AND person_id=? AND role='executor'
    `, open.assignment.id, volunteer.id).n, 1);
    ...
    addDocument(database, workspace.id, 'doc_report', 'ver_report', 'report.pdf', {
      extractedText: 'ОТЧЁТ\nПодготовить отчёт выполнено на 100 процентов.'
    });
    const submitted = attachAssignmentReport(database, workspace.id, assignmentId, {
      documentId: 'doc_report', actorPersonId: executor.id, note: 'Итоговый отчёт'
    }, '2026-10-19T12:00:00.000Z');
    assert.equal(submitted.status, 'submitted');
    assert.ok(listPlanFact(database, workspace.id, { limit: 100 }, new Date('2026-10-19T13:00:00Z')).items
      .some((item) => item.id === assignmentId && item.status === 'submitted'));

    const requisitesOnly = createSupportingDocument(database, workspace.id, {
      documentNumber: '12-03/26', documentDate: '2026-10-19', title: 'Подтверждение выполнения',
      targetKind: 'plan_item', targetId: assigned.id, relationKind: 'completion'
    }, executor.id, '2026-10-19T12:05:00.000Z');
    ...
    const science = createScientificItem(database, workspace.id, {
      title: 'Статья по результатам работы', kind: 'article', authors: ['Иванов Иван Иванович'], publicationYear: 2026
    }, '2026-10-19T12:10:00.000Z');
    const fileSupport = createSupportingDocument(database, workspace.id, {
      documentNumber: 'PUB-2026-7', documentDate: '2026-10-18', title: 'Справка о публикации',
      documentVersionId: 'ver_report', targetKind: 'scientific_item', targetId: science.id, relationKind: 'publication'
    }, executor.id, '2026-10-19T12:11:00.000Z');
