...
async function legacyMigrations(root) {
  ...
  for (const name of files) {
    const version = Number(name.match(/^(\d+)/u)?.[1] || 0);
    if (version <= 19) await copyFile(join(migrationsDir, name), join(target, basename(name)));
  }
  return target;
}
...
test('обновление 19 → 20 сохраняет планы, сотрудников и научные карточки', async () => {
  ...
  const oldMigrations = await legacyMigrations(root);
