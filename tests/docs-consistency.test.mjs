...
async function fixture() {
  ...
  await writeFile(join(root, 'VERSION'), '0.3.2\n');
  await writeFile(join(root, 'package.json'), JSON.stringify({
    version: '0.3.2', scripts: { check: 'node check.mjs', good: 'node good.mjs' }
  }));
  ...
  await writeFile(join(root, 'docs', 'releases', '0.3.2.md'), '# Kafedra Planner 0.3.2\n');
  await writeFile(join(root, 'README.en.md'), '> Current milestone: **`0.3.2`**\n');
  await writeFile(join(root, 'docs', 'ROADMAP.md'), '## Текущий рубеж — `0.3.2`\n');
  await writeFile(join(root, 'docs', 'RELEASE_CANDIDATE.md'), '# Release candidate 0.3.2\n');
  await writeFile(join(root, 'docs', 'VALIDATION.md'), 'Актуальный рубеж: `0.3.2`\n');
  await writeFile(join(root, 'docs', 'UX_FLOWS.md'), 'Статус: рабочие контуры версии `0.3.2`\n');
  await writeFile(join(root, '.github', 'workflows', 'release-gate.yml'), 'name: Release gate 0.3.2\n');
  await writeFile(join(root, '.github', 'workflows', 'release.yml'), 'workflows: ["Release gate 0.3.2"]\n');
  return root;
}
...
test('documentation checker reports drift between VERSION and current release markers', async () => {
  const root = await fixture();
  await writeFile(join(root, 'README.md'), '> Текущий рубеж: **`0.3.1`**\n');
  await writeFile(join(root, 'docs', 'ROADMAP.md'), '## Текущий рубеж — `0.3.1`\n');
  const errors = await checkDocumentation({ root });
  const releaseErrors = errors.filter((item) => item.kind === 'release-version');
  assert.equal(releaseErrors.length, 2);
  assert.ok(releaseErrors.some((item) => item.file === 'README.md' && item.target === '0.3.1'));
  assert.ok(releaseErrors.some((item) => item.file === 'docs/ROADMAP.md' && item.target === '0.3.1'));
});
