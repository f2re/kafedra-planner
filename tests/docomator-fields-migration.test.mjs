...
test('обновление 29 → 30 добавляет выбор полей и сохраняет связи Оформлятора и библиотеку заседаний', async () => {
  ...
  try {
    ...
    assert.ok(database.get("SELECT name FROM sqlite_master WHERE type='table' AND name='meeting_template_catalog'"));
    assert.ok(database.get("SELECT name FROM sqlite_master WHERE type='table' AND name='meeting_template_test_runs'"));
  } finally {
    database.close();
  }
