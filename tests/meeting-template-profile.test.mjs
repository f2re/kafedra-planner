...
function tableTemplateXml() {
  ...
  const row = `<w:tr>${cell('1. Вопрос повестки', '<w:b/>')}${cell('СЛУШАЛИ: Текст слушали', '<w:i/>')}${cell('РЕШИЛИ: Текст решения', '<w:u w:val="single"/>')}</w:tr>`;
  return documentXml(`${globals}<w:tbl><w:tblPr/><w:tblGrid><w:gridCol w:w="1800"/><w:gridCol w:w="1800"/><w:gridCol w:w="1800"/></w:tblGrid>${row}</w:tbl>`);
}
...
test('строка таблицы повторяется для двух вопросов и сохраняет оформление ячеек', async () => {
  ...
  try {
    ...
    assert.equal((rendered.match(/<w:tcW w:w="1800"/gu) || []).length, 6);
    assert.match(rendered, /Об утверждении плана/u);
    assert.match(rendered, /О научной работе/u);
    assert.equal((rendered.match(/<w:i\/>/gu) || []).length, 2);
    assert.equal((rendered.match(/<w:u w:val="single"\/>/gu) || []).length, 2);
...
test('сервер отклоняет устаревший structural hash и пересекающиеся диапазоны', async () => {
  const root = await mkdtemp(join(tmpdir(), 'kafedra-meeting-profile-validation-'));
  ...
  const config = { blobDir, tempDir, maxUploadBytes: 20 * 1024 * 1024 };
  try {
    const workspace = ensureDefaultWorkspace(database);
    await writeDocx(templatePath, paragraphTemplateXml());
    const uploaded = await uploadMeetingTemplate(database, config, workspace.id, createReadStream(templatePath), {
      kind: 'protocol', originalName: 'Проверяемый протокол.docx'
    });
    const analysis = await analyzeMeetingTemplate(database, workspace.id, uploaded.version_id, 'protocol');
    await assert.rejects(
      saveMeetingTemplateProfile(database, config, workspace.id, uploaded.version_id, 'protocol', {
        structureSha256: 'outdated', bindings: analysis.suggestions
      }),
      assertCode('meeting_template_structure_changed')
    );
    const number = analysis.suggestions.find((binding) => binding.field === 'protocol_number');
    await assert.rejects(
      saveMeetingTemplateProfile(database, config, workspace.id, uploaded.version_id, 'protocol', {
        structureSha256: analysis.structureSha256,
        bindings: [number, { ...number, field: 'meeting_date' }]
      }),
      assertCode('meeting_template_ranges_overlap')
    );
  } finally {
    database.close();
    await rm(root, { recursive: true, force: true });
...
test('черновик и готовый профиль сохраняются неизменяемыми редакциями, повтор готового запроса не создаёт дубль', async () => {
  ...
  try {
    ...
    const analysis = await analyzeMeetingTemplate(database, workspace.id, uploaded.version_id, 'protocol');
    const draft = await saveMeetingTemplateProfile(database, config, workspace.id, uploaded.version_id, 'protocol', {
      structureSha256: analysis.structureSha256,
      bindings: [analysis.suggestions.find((binding) => binding.field === 'protocol_number')]
    });
    assert.equal(draft.status, 'draft');
    assert.ok(draft.missingFields.includes('decision'));
    assert.equal(latestMeetingTemplateProfile(database, workspace.id, uploaded.version_id, 'protocol', true), null);

    const first = await saveMeetingTemplateProfile(database, config, workspace.id, uploaded.version_id, 'protocol', {
      structureSha256: analysis.structureSha256,
      bindings: analysis.suggestions
    });
    ...
    const second = await saveMeetingTemplateProfile(database, config, workspace.id, uploaded.version_id, 'protocol', {
      structureSha256: analysis.structureSha256,
      bindings: analysis.suggestions
    });
    assert.equal(second.duplicateRequest, true);
    assert.equal(second.profile_version_id, first.profile_version_id);
    assert.equal(database.get("SELECT COUNT(*) AS n FROM documents WHERE document_type = 'meeting_template_profile'").n, 2);
