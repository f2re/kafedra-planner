...
while IFS= read -r line; do
  ...
  case "$entry_type" in
    '-'|'d') ;;
    *) echo "Неподдерживаемый тип записи в release-архиве: $entry_type" >&2; exit 5 ;;
  esac
...
while IFS= read -r entry; do
  ...
  if [[ "$entry" =~ [[:cntrl:]\\] ]]; then
    echo "Неподдерживаемое имя файла в архиве: $entry" >&2
    exit 6
  fi
done < "$LIST_FILE"
...
REQUIRED_FILES=(
  ...
  application/VERSION
  application/apps/api/src/main.mjs
  application/apps/worker/src/main.mjs
  application/scripts/smoke-test.mjs
  application/scripts/system-preflight.mjs
  application/scripts/offline/runtime-contract.mjs
  application/deploy/systemd/kafedra-planner-api.service
  application/deploy/systemd/kafedra-planner-worker.service
)
for relative_path in "${REQUIRED_FILES[@]}"; do
  [[ -f "$ROOT/$relative_path" ]] || { echo "Комплект неполон: отсутствует $relative_path" >&2; exit 8; }
done
for relative_dir in application/packages application/public application/migrations application/docs; do
  [[ -d "$ROOT/$relative_dir" ]] || { echo "Комплект неполон: отсутствует каталог $relative_dir" >&2; exit 8; }
done
...
) > "$WORK_DIR/manifest-check.txt" 2>&1; then
  cat "$WORK_DIR/manifest-check.txt" >&2
  exit 9
fi
...
"$EMBEDDED_NODE" \
  "$ROOT/application/scripts/offline/runtime-contract.mjs" verify-bundle \
  --root "$ROOT"
