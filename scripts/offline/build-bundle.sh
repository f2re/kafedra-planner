...
cleanup() {
  ...
  trap - EXIT
  rm -rf "$WORK_DIR"
  if [[ -n "$TEMP_ARCHIVE" ]]; then rm -f "$TEMP_ARCHIVE" "$TEMP_ARCHIVE.sha256"; fi
  exit "$status"
}
trap cleanup EXIT
log() { printf '%s\n' "$*" >&2; }
fail() { log "Ошибка сборки offline bundle: $*"; exit 2; }
for command in tar gzip sha256sum find sort xargs install cp grep head readlink awk mktemp; do command -v "$command" >/dev/null 2>&1 || fail "не найдена команда $command"; done
...
if [[ -n "${XDG_CACHE_HOME:-}" ]]; then DEFAULT_CACHE_HOME="$XDG_CACHE_HOME"; elif [[ -n "${HOME:-}" ]]; then DEFAULT_CACHE_HOME="$HOME/.cache"; else DEFAULT_CACHE_HOME="$ROOT/.cache"; fi
...
candidate_license() {
  ...
  for candidate in "${NODE_LICENSE_FILE:-}" "$home/LICENSE" "$home/LICENSE.md"; do [[ -z "$candidate" || ! -f "$candidate" ]] || { printf '%s\n' "$candidate"; return 0; }; done
  return 1
}
runtime_has_external_libnode() { local runtime="$1" output; command -v ldd >/dev/null 2>&1 || return 1; output="$(ldd "$runtime" 2>&1 || true)"; grep -Eq '(^|[[:space:]/])libnode\.so' <<<"$output"; }
runtime_is_usable() {
  local runtime="$1"
  [[ -x "$runtime" ]] || return 1
  candidate_license "$runtime" >/dev/null || return 1
  runtime_has_external_libnode "$runtime" && return 1
  "$runtime" "$ROOT/scripts/offline/runtime-contract.mjs" inspect --package-json "$ROOT/package.json" --runtime "$runtime" >/dev/null 2>&1
}
download_file() {
  local url="$1" output="$2"
  if command -v curl >/dev/null 2>&1; then curl --fail --location --silent --show-error --retry 3 --connect-timeout 15 --output "$output" "$url"
  elif command -v wget >/dev/null 2>&1; then wget -q --tries=3 --timeout=30 -O "$output" "$url"
  else fail "для автоматической загрузки Node.js нужен curl или wget; либо задайте NODE_RUNTIME_DIR"; fi
}
prepare_cached_runtime() {
  ...
  (cd "$download_dir"; sha256sum -c --strict runtime.sha256 >/dev/null) || fail "SHA-256 Node.js runtime не совпадает с закреплённым digest из package.json"
...
if [[ -n "${NODE_RUNTIME_DIR:-}" ]]; then EXPLICIT_RUNTIME=true; NODE_SOURCE="$NODE_RUNTIME_DIR/bin/node"
elif [[ -n "${NODE_BINARY:-}" ]]; then EXPLICIT_RUNTIME=true; NODE_SOURCE="$NODE_BINARY"
else
  ...
  if ! runtime_is_usable "$NODE_SOURCE"; then
    ...
    if ! runtime_is_usable "$NODE_SOURCE"; then rm -rf "$CACHE_DIR"; prepare_cached_runtime "$CACHE_DIR"; fi
  fi
fi
...
if ! runtime_is_usable "$NODE_SOURCE"; then
  [[ "$EXPLICIT_RUNTIME" != true ]] || fail "NODE_RUNTIME_DIR/NODE_BINARY не соответствует закреплённому runtime v$TARGET_NODE_VERSION и engines.node"
...
if command -v git >/dev/null 2>&1 && git -C "$ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  if [[ -n "$DIRTY_TREE" && "${ALLOW_DIRTY_BUNDLE:-false}" != "true" ]]; then fail "рабочее дерево содержит незакоммиченные изменения; release должен однозначно соответствовать commit (для диагностической сборки: ALLOW_DIRTY_BUNDLE=true)"; fi
  if [[ -z "$DIRTY_TREE" ]]; then GIT_COMMIT="$(git -C "$ROOT" rev-parse HEAD)"; else log "Внимание: разрешена диагностическая сборка из грязного дерева; gitCommit будет unknown."; fi
fi

mkdir -p "$OUT_DIR" "$BUNDLE_ROOT/application" "$BUNDLE_ROOT/runtime/node/bin"
cp -a "$ROOT/apps" "$ROOT/packages" "$ROOT/public" "$ROOT/migrations" "$ROOT/scripts" "$ROOT/deploy" "$ROOT/docs" "$ROOT/package.json" "$ROOT/VERSION" "$ROOT/README.md" "$ROOT/SECURITY.md" "$ROOT/.env.example" "$BUNDLE_ROOT/application/"
...
find "$BUNDLE_ROOT/application" -type f \( -name '*.pyc' -o -name '*.pyo' -o -name '*.log' \) -delete

FULL_BUNDLE=false
if [[ -n "${PYTHON_RUNTIME_DIR:-}" || -n "${OS_PACKAGES_DIR:-}" || "${REQUIRE_FULL_BUNDLE:-false}" == "true" ]]; then
  FULL_BUNDLE=true
  [[ -n "${PYTHON_RUNTIME_DIR:-}" && -x "$PYTHON_RUNTIME_DIR/python" && -f "$PYTHON_RUNTIME_DIR/runtime.json" ]] || fail "full bundle требует PYTHON_RUNTIME_DIR с managed Python"
  [[ -n "${OS_PACKAGES_DIR:-}" && -d "$OS_PACKAGES_DIR" ]] || fail "full bundle требует OS_PACKAGES_DIR с замкнутым набором .deb"
  source "$ROOT/scripts/offline/lib.sh"
...
"$BUNDLE_ROOT/runtime/node/bin/node" "$BUNDLE_ROOT/application/scripts/offline/runtime-contract.mjs" write-release --package-json "$BUNDLE_ROOT/application/package.json" --version-file "$BUNDLE_ROOT/application/VERSION" --runtime "$BUNDLE_ROOT/runtime/node/bin/node" --output "$BUNDLE_ROOT/release.json" --git-commit "$GIT_COMMIT" >/dev/null
...
(cd "$BUNDLE_ROOT"; LC_ALL=C find . -type f ! -path './manifest.sha256' -print0 | LC_ALL=C sort -z | xargs -0 sha256sum > manifest.sha256)
ARCHIVE_TAG="${FULL_BUNDLE_TAG:-}"
if [[ -n "$ARCHIVE_TAG" ]]; then [[ "$ARCHIVE_TAG" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || fail "небезопасный FULL_BUNDLE_TAG: $ARCHIVE_TAG"; ARCHIVE_BASENAME="kafedra-planner-$VERSION-$ARCHIVE_TAG.tar.gz"; else ARCHIVE_BASENAME="kafedra-planner-$VERSION.tar.gz"; fi
...
[[ -z "${SOURCE_DATE_EPOCH:-}" ]] || TAR_OPTIONS+=(--mtime="@$SOURCE_DATE_EPOCH")
tar "${TAR_OPTIONS[@]}" -C "$WORK_DIR" -cf - "kafedra-planner-$VERSION" | gzip -n -6 > "$TEMP_ARCHIVE"
(cd "$OUT_DIR"; sha256sum "$(basename "$TEMP_ARCHIVE")" > "$(basename "$TEMP_ARCHIVE").sha256")
log "Проверка собранного автономного комплекта..."
REQUIRE_ARCHIVE_SHA256=true SKIP_SYSTEM_PREFLIGHT=true "$ROOT/scripts/offline/verify-bundle.sh" "$TEMP_ARCHIVE" >&2
mv -f "$TEMP_ARCHIVE" "$ARCHIVE"
rm -f "$TEMP_ARCHIVE.sha256"; TEMP_ARCHIVE=""
(cd "$OUT_DIR"; sha256sum "$ARCHIVE_BASENAME" > "$ARCHIVE_BASENAME.sha256.tmp"; mv -f "$ARCHIVE_BASENAME.sha256.tmp" "$ARCHIVE_BASENAME.sha256"; sha256sum -c --strict "$ARCHIVE_BASENAME.sha256" >/dev/null)
log "Offline bundle собран: $ARCHIVE ($(du -h "$ARCHIVE" | awk '{print $1}')); профиль=$([[ "$FULL_BUNDLE" == true ]] && echo full || echo runtime-only); Node.js v$TARGET_NODE_VERSION"
printf '%s\n' "$ARCHIVE"
