...
ON organization_units(workspace_id, parent_id, status, valid_from, valid_to, sort_order, name);
...
CREATE TABLE person_appointments (
  ...
  workload_fraction REAL NOT NULL DEFAULT 1.0 CHECK(workload_fraction > 0 AND workload_fraction <= 1.5),
...
CREATE INDEX idx_person_appointments_person_period ON person_appointments(workspace_id, person_id, valid_from, valid_to, status);
CREATE INDEX idx_person_appointments_unit_period ON person_appointments(workspace_id, organization_unit_id, valid_from, valid_to, status);
CREATE INDEX idx_person_appointments_position ON person_appointments(workspace_id, position_id, status);
CREATE INDEX idx_person_appointments_manager ON person_appointments(workspace_id, manager_person_id, valid_from, valid_to, status);

CREATE TRIGGER person_appointment_unit_period_insert
BEFORE INSERT ON person_appointments
WHEN EXISTS (
  SELECT 1 FROM organization_units u WHERE u.id = NEW.organization_unit_id
    AND (NEW.valid_from < u.valid_from OR COALESCE(NEW.valid_to,'9999-12-31') > COALESCE(u.valid_to,'9999-12-31'))
)
BEGIN SELECT RAISE(ABORT, 'organization_unit_period_mismatch'); END;

CREATE TRIGGER person_appointment_unit_period_update
BEFORE UPDATE OF organization_unit_id, valid_from, valid_to ON person_appointments
WHEN EXISTS (
  SELECT 1 FROM organization_units u WHERE u.id = NEW.organization_unit_id
    AND (NEW.valid_from < u.valid_from OR COALESCE(NEW.valid_to,'9999-12-31') > COALESCE(u.valid_to,'9999-12-31'))
)
...
CREATE INDEX idx_organization_unit_managers_period ON organization_unit_managers(workspace_id, organization_unit_id, valid_from, valid_to, status);
CREATE INDEX idx_organization_unit_managers_person ON organization_unit_managers(workspace_id, person_id, valid_from, valid_to, status);

CREATE TRIGGER organization_manager_unit_period_insert
BEFORE INSERT ON organization_unit_managers
WHEN EXISTS (
  SELECT 1 FROM organization_units u WHERE u.id = NEW.organization_unit_id
    AND (NEW.valid_from < u.valid_from OR COALESCE(NEW.valid_to,'9999-12-31') > COALESCE(u.valid_to,'9999-12-31'))
)
...
CREATE INDEX idx_scientific_author_affiliations_person ON scientific_author_affiliations(workspace_id, person_id, valid_on, scientific_item_id);
CREATE INDEX idx_scientific_author_affiliations_unit ON scientific_author_affiliations(workspace_id, organization_unit_id, valid_on, scientific_item_id);

INSERT INTO organization_units(
  id, workspace_id, parent_id, code, name, unit_kind, valid_from, valid_to, status, sort_order,
  created_by_person_id, created_at, updated_at
)
SELECT 'unit_legacy_' || w.id, w.id, NULL, 'legacy-root',
  COALESCE(NULLIF(trim(w.name), ''), 'Организация'), 'organization',
  COALESCE(substr(w.created_at, 1, 10), '2000-01-01'), NULL, 'active', 0, NULL,
  COALESCE(w.created_at, CURRENT_TIMESTAMP), COALESCE(w.created_at, CURRENT_TIMESTAMP)
FROM workspaces w
WHERE EXISTS (SELECT 1 FROM people p WHERE p.workspace_id = w.id AND length(trim(COALESCE(p.position, ''))) > 0);

INSERT INTO person_appointments(
  id, workspace_id, person_id, organization_unit_id, position_id, position_title_snapshot,
  manager_person_id, appointment_kind, workload_fraction, valid_from, valid_to, status,
  source_document_version_id, evidence_json, created_by_person_id, created_at, updated_at
)
SELECT 'appt_legacy_' || p.id, p.workspace_id, p.id, 'unit_legacy_' || p.workspace_id, NULL,
  trim(p.position), p.manager_id, 'primary', 1.0,
  substr(COALESCE(p.created_at, CURRENT_TIMESTAMP), 1, 10), NULL,
  CASE WHEN p.status = 'inactive' THEN 'ended' ELSE 'active' END, NULL,
  json_object('source', 'legacy_people', 'position', p.position, 'managerPersonId', p.manager_id, 'migratedAt', CURRENT_TIMESTAMP),
  NULL, COALESCE(p.created_at, CURRENT_TIMESTAMP), CURRENT_TIMESTAMP
FROM people p
...
WITH author_dates AS (
  SELECT sia.scientific_item_id, sia.author_raw, sia.person_id, si.workspace_id,
    COALESCE(substr(si.published_at, 1, 10), CASE WHEN si.publication_year IS NOT NULL THEN printf('%04d-12-31', si.publication_year) END,
      substr(si.created_at, 1, 10), date('now')) AS valid_on
  FROM scientific_item_authors sia JOIN scientific_items si ON si.id = sia.scientific_item_id
  WHERE sia.person_id IS NOT NULL
), resolved AS (
  SELECT ad.*, (SELECT pa.id FROM person_appointments pa
    WHERE pa.workspace_id = ad.workspace_id AND pa.person_id = ad.person_id AND pa.status <> 'cancelled'
      AND pa.valid_from <= ad.valid_on AND (pa.valid_to IS NULL OR pa.valid_to >= ad.valid_on)
...
SELECT 'scienceaff_' || lower(hex(randomblob(16))), r.workspace_id, r.scientific_item_id,
  ...
  json_object('source', 'organization_migration', 'appointmentId', pa.id), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM resolved r
...
BEGIN
  ...
  SELECT 'scienceaff_' || lower(hex(randomblob(16))), si.workspace_id, NEW.scientific_item_id,
    ...
    'derived', json_object('source', 'organization_trigger', 'appointmentId', pa.id), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
  ...
  LEFT JOIN person_appointments pa ON pa.id = (
    ...
    WHERE candidate.workspace_id = si.workspace_id AND candidate.person_id = NEW.person_id
      ...
      AND candidate.valid_from <= COALESCE(substr(si.published_at, 1, 10), CASE WHEN si.publication_year IS NOT NULL THEN printf('%04d-12-31', si.publication_year) END,
        substr(si.created_at, 1, 10), date('now'))
      AND (candidate.valid_to IS NULL OR candidate.valid_to >= COALESCE(substr(si.published_at, 1, 10), CASE WHEN si.publication_year IS NOT NULL THEN printf('%04d-12-31', si.publication_year) END,
        substr(si.created_at, 1, 10), date('now')))
