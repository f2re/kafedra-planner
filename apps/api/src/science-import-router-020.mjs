...
function requireManager(context) {
  ...
  if (!['admin','manager'].includes(context.role)) {
    throw new AppError('science_import_access_forbidden', 'Массовый импорт выполняет руководитель или администратор.', 403);
  }
}
...
function canReadRun(database, workspaceId, context, run) {
  if (!run?.source_document_id) return false;
  try {
    assertObjectAccess(database, workspaceId, context, 'document', run.source_document_id, 'read');
    return true;
  } catch { return false; }
}

export function createScienceImportRouter020({ database }) {
  return async function routeScienceImport020(request, response, url) {
    ...
    try {
      if (path === '/api/science-imports' && method === 'GET') {
        const items = listScienceImportRuns(database, workspace, url.searchParams.get('limit') || 100)
      ...
      const match = path.match(/^\/api\/science-imports\/([^/]+)$/u);
      if (match && method === 'GET') {
        const run = getScienceImportRun(database, workspace, decodeURIComponent(match[1]));
        if (!run) throw new AppError('science_import_not_found', 'Импорт не найден.', 404);
        if (!canReadRun(database, workspace, context, run)) {
          throw new AppError('science_import_access_forbidden', 'Нет доступа к источнику импорта.', 403);
        }
