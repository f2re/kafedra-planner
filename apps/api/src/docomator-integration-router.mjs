...
function integrationError(error) {
  ...
  const messages = {
    ...
    docomator_dns_error: ['Имя сервера Оформлятора не найдено. Проверьте адрес или локальный DNS.', 502],
    ...
    docomator_tls_error: ['Не удалось установить защищённое соединение с Оформлятором. Проверьте HTTPS и сертификат.', 502],
...
function saveCombinedSettings(database, workspace, input) {
  database.transaction(() => {
    saveDocomatorSettings(database, workspace, input);
    saveDocomatorFieldMapping(database, workspace, input);
  });
  return combinedSettings(database, workspace);
}

function recordSuccessfulCheck(database, workspace, input, result) {
  database.transaction(() => {
  ...
  return async function routeDocomatorIntegration(request, response, url) {
    ...
    try {
      ...
      if (path === '/api/integrations/docomator' && method === 'PUT') {
        ...
        return sendJson(response, 200, saveCombinedSettings(database, workspace, input));
      }
      if (path === '/api/integrations/docomator/check' && method === 'POST') {
        ...
        try {
          ...
          return sendJson(response, 200, {
            ...result,
            ...fieldInfo,
            settings: recordSuccessfulCheck(database, workspace, input, result)
          });
        } catch (error) {
