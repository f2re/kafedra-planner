...
export function planLifecycleEmptyMessage(mode) {
  return normalizePlanLifecycleMode(mode) === 'archived'
...
function closestTab(target, segment) {
  const element = target?.nodeType === 1 ? target : target?.parentElement;
  const tab = element?.closest?.('[role="tab"][data-lifecycle-value]');
  return tab && segment.contains(tab) ? tab : null;
}
...
function createSegment(select) {
  ...
  segment.setAttribute('aria-orientation', 'horizontal');
  ...
  segment.addEventListener('click', (event) => {
    const button = closestTab(event.target, segment);
    if (!button) return;
...
function reconcile(doc = document) {
  const select = doc.getElementById(SOURCE_ID);
  if (!isSelect(select)) return null;
