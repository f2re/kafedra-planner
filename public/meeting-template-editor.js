...
function paragraphStyle(style = {}) {
  ...
  const marginLeft = safeNumber(style.marginLeftPt);
  const marginRight = safeNumber(style.marginRightPt);
  const firstLine = safeNumber(style.firstLinePt);
  const hanging = safeNumber(style.hangingPt);
  const before = safeNumber(style.spaceBeforePt, 0);
  const after = safeNumber(style.spaceAfterPt, 0);
  const line = safeNumber(style.linePt, 0);
  if (marginLeft !== null) declarations.push(`margin-left:${marginLeft}pt`);
  if (marginRight !== null) declarations.push(`margin-right:${marginRight}pt`);
  if (firstLine !== null) declarations.push(`text-indent:${firstLine}pt`);
  if (hanging !== null && firstLine === null) declarations.push(`text-indent:${-hanging}pt`);
  if (before !== null) declarations.push(`padding-top:${before}pt`);
...
function fieldOptions(selected = '') {
  const groups = [
    ['document', 'Поля заседания'],
    ['agenda', 'Повторяемый вопрос']
  ];
...
function assignSelection() {
  ...
  editorState.bindings = editorState.bindings.filter((binding) => binding.field !== field);
  const overlap = editorState.bindings.some((binding) =>
  ...
  editorState.bindings.push(candidate);
  editorState.bindings.sort((left, right) => left.field.localeCompare(right.field, 'ru'));
  ...
  try {
    const profile = await meetingApi(
      `/api/meeting-templates/${encodeURIComponent(editorState.versionId)}/profiles?kind=${encodeURIComponent(editorState.kind)}`,
      {
        ...
        body: JSON.stringify({
          structureSha256: editorState.analysis.structureSha256,
          bindings: editorState.bindings
        })
      }
    );
...
function bindEditorEvents() {
  ...
  modal.addEventListener('click', (event) => {
    ...
    if (event.target.closest?.('[data-template-editor-back]') && typeof editorState.onBack === 'function') {
      editorState.onBack();
    }
  });
}
...
export async function openMeetingTemplateEditor({ kind, versionId, title, onBack, onSaved }) {
  ...
  try {
    ...
    const seed = analysis.latestProfile?.bindings?.length
      ? analysis.latestProfile.bindings
