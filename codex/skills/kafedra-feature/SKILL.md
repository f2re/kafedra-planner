...
For substantial user-facing or document-workspace implementation, consume the route selected by `kafedra-workspace-orchestrator` and preserve the routed invariants. In particular use `kafedra-states-and-recovery` for async/partial/retry behavior, `kafedra-provenance-and-inspector` for source/evidence continuity, and `kafedra-plan-calendar-continuity` when work crosses plans, assignments, calendar or Plan/Fact. Reusable profile guidance never authorizes a parallel entity or wider GRACE scope.
