...
When `kafedra-workspace-orchestrator` routes document-workspace concerns, translate `kafedra-ux-acceptance` into executable regression evidence. Use `kafedra-states-and-recovery` for async, partial-success, cancellation and targeted-retry cases, and verify provenance/idempotency when the routed profile identifies them. These reusable checks supplement rather than replace project/domain/release gates.
