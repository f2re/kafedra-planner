...
Kafedra Planner is an offline-first daily work system for an academic
department: calendar, annual plans, assignments, documents, meetings,
reporting, research activity, and auditable evidence.

> Current milestone: **`0.2.0`**, SQLite schema **25**. Core workflows do not
> require Internet access, an LLM, Docker, or cloud services. This line remains
> a prerelease until the real Astra Linux/Debian installation, upgrade,
> restoration, and rollback acceptance in [TARGET_ACCEPTANCE.md](docs/TARGET_ACCEPTANCE.md) and issue #27 is complete.
...
An uploaded PDF/DOCX/ODT/XLSX/ODS file or scan is never replaced by recognised
text or an AI result. Every extracted fact retains its source and locator;
manual corrections keep their reason and history.
...
A failing file, adapter, OCR component, or optional converter must not block
other documents or the core daily workflow.
...
For a Debian 12 `amd64` target, download the four files from the selected
[GitHub Release](https://github.com/f2re/kafedra-planner/releases), keep them
in one directory, verify the checksum, and run:
...
The installer verifies the archive and manifest, runs migrations, creates a
verified pre-update backup, and rolls back an unsuccessful update. At the first
open, the user sets a four-digit PIN. See the detailed [release guide](docs/GITHUB_RELEASES.md), [offline install contract](docs/OFFLINE_INSTALL.md), and [backup/restore procedure](docs/BACKUP_RESTORE.md).

For Astra Linux, another architecture, or another OS series, build a bundle on
a compatible Debian/Astra reference machine instead:
...
The full list of browser, release, and deployment checks is in the Russian
[README](README.md#разработка-и-проверка). The project is intentionally
Russian-first; help translating user-facing documentation or the interface is
welcome through an issue or pull request.
